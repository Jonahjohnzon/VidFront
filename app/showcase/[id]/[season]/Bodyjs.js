...
const Bodyjs = ({params}) => {
  ...
  const div = 15
    ...
    const Result = async()=>{
      try{
        const data = await fetch(`${backend}/getMovies/${id}?start=${start}&limit=${limit}&season=${sea}`)
  ...
  <div className=' w-12 h-12 rounded-full border-b-4 border-b-[#03091A] transform -scale-y-180 border-red-600 border-x-4 border-t-4 animate-spin'></div>
    ...
    <div className='2xl:w-[56%] w-[80%] '>
      ...
      <Commentss url={`${backend}/postComment/${id}`} forum={false} setreply={setreply} reply={reply}  comment_display={comment_display} setDispay={setDispay}  setload={setload} Check={Check}  setDisable={ setDisable} disable={disable} Result={Result}/>
